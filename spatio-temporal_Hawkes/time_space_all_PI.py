...
# ========== 2. 加载多个国家的行政区划 ==========
# 定义国家列表及其 Shapefile 路径
countries = [
    ('Israel', 'gadm41_ISR_shp/gadm41_ISR_1.shp'),
    ('Palestine', 'gadm41_PSE_shp/gadm41_PSE_1.shp'),
    ('Lebanon', 'gadm41_LBN_shp/gadm41_LBN_1.shp'),
    ('Syria', 'gadm41_SYR_shp/gadm41_SYR_1.shp'),
    ('Palestine', 'gadm41_IRN_shp/gadm41_IRN_1.shp'),
    ('Yemen', 'gadm41_YEM_shp/gadm41_YEM_1.shp'),
]

# 注意：巴以冲突主要集中在 Israel 和 Palestine
# 如果只想看核心区域，可以只保留前两个
primary_countries = ['Israel', 'Palestine']  # 主要分析区域

# 加载所有国家的行政区
all_regions = []
country_names = []

for country_name, shp_path in countries:
    if not os.path.exists(shp_path):
        print(f"警告: 文件不存在 {shp_path}，跳过 {country_name}")
        continue
    ...
    try:
        ...
        # 查找名称字段
        name_field = None
        for possible in ['NAME_1', 'NAME_2', 'ADM1', 'ADM2', 'NAME']:
            if possible in field_names:
                name_field = possible
                break

        print(f"加载 {country_name}: {len(sf.shapes())} 个行政区")

        for idx, shape in enumerate(sf.shapes()):
            parts_coords = []
            parts_idx = list(shape.parts) + [len(shape.points)]
            for i in range(len(parts_idx)-1):
                lons = [shape.points[j][0]
                        for j in range(parts_idx[i], parts_idx[i+1])]
                lats = [shape.points[j][1]
                        for j in range(parts_idx[i], parts_idx[i+1])]
                parts_coords.append((lons, lats))

            rec = sf.record(idx)
            if name_field:
                region_name = rec[field_names.index(name_field)]
            else:
                region_name = f"{country_name}_Reg{idx}"
        ...
        print(f"加载 {country_name} 失败: {e}")

n_regions = len(all_regions)
print(f"\n共加载 {n_regions} 个行政区")

# 预计算中心点矩阵
centers = np.array([r['center'] for r in all_regions])
...
for r in all_regions:
    region_bbox.append(r['bbox'])

for i, row in df.iterrows():
    lon, lat = row['longitude'], row['latitude']
    for rid, r in enumerate(all_regions):
        bminx, bmaxx, bminy, bmaxy = region_bbox[rid]
        if not (bminx <= lon <= bmaxx and bminy <= lat <= bmaxy):
            continue
        matched = False
        for plon, plat in r['parts']:
            if point_in_polygon(lon, lat, plon, plat):
                df.at[i, 'region_id'] = rid
                matched = True
                break
        if matched:
            break
...
# ========== 5. 构建时间序列 ==========
Y_region = np.zeros((T, n_regions), dtype=np.float64)
for _, row in df.iterrows():
    Y_region[int(row['t']), int(row['region_id'])] += 1

active_regions = np.where(Y_region.sum(axis=0) > 0)[0]
print(f"有事件的区: {len(active_regions)}/{n_regions}")
...
def neg_log_lik_national(params, Y_matrix, dist_matrix):
    ...
    lam0 = np.exp(mu0)
    T_len, N_regions = Y_matrix.shape
...
mean_daily_events = np.mean(Y_region)
...
def neg_log_lik_null(mu0):
    if mu0 > 10 or mu0 < -10:
        return 1e10
    lam0 = np.exp(mu0)
    log_lik = np.sum(Y_region * np.log(lam0) - lam0 - gammaln(Y_region + 1))
    return -log_lik
...
p_value = 0.5 * (1 - chi2.cdf(LR, df=3)) + 0.5 * (1 - chi2.cdf(LR, df=4))
print(p_value)
print("\n" + "="*70)
print("似然比检验结果")
print("="*70)
...
if p_value < 0.001:
    print("\n✓ 在 0.001 显著性水平下拒绝 H₀")
    print("  巴以冲突地区存在极显著的空间自激发/传染效应！")
elif p_value < 0.05:
    print("\n✓ 在 0.05 显著性水平下拒绝 H₀")
    print("  巴以冲突地区存在显著的空间自激发效应")
else:
    print("\n✗ 不拒绝 H₀")
    print("  无显著的空间自激发效应")
...
Y_total = Y_region.sum(axis=1)

# 左图：实际事件数
ax1 = axes[0]
ax1.plot(Y_total, alpha=0.7, color='steelblue', linewidth=0.8, label='每日实际袭击次数')
ax1.set_xlabel('天数')
ax1.set_ylabel('每日总袭击次数')
ax1.set_title(f'巴以冲突地区每日总事件次数\n(总计 {Y_total.sum():.0f} 次)')
...
plt.tight_layout()
plt.savefig('S-T_output/Israel_Palestine_Global_Hawkes_Test.png', dpi=150)
plt.show()

print("图片已成功更新并保存: S-T_output/Israel_Palestine_Global_Hawkes_Test.png")
...
for _, row in df.iterrows():
    ...
    country = all_regions[rid]['country']
...
print(f"空间带宽 σ = {sigma_opt:.2f} km")
print(f"时间衰减 β = {beta_opt:.4f}")
print(f"似然比 LR = {LR:.4f}")
print(f"p-value = {p_value:.6e}")

if p_value < 0.05:
    print("\n结论：巴以冲突地区的袭击事件具有显著的时空传染/自激特征。")
    print("       一次袭击会显著提高周围区域在未来短期内的袭击概率。")
    print(f"       空间影响范围约 {sigma_opt:.0f} km，时间记忆约 {1/(1-beta_opt):.1f} 天。")
else:
    print("\n结论：巴以冲突地区的袭击事件未检测到显著的时空传染效应。")
